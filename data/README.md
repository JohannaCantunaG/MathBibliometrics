...
└── Statistics/
```
Place every CSV exported from Scopus inside its corresponding subject folder.
...
Scopus is a dynamic database that is continuously updated. Consequently, executing the same search query at a different date may return a different number of records due to newly indexed publications, metadata corrections, or database updates. 
